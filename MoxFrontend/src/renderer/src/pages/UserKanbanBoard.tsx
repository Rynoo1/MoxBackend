  ...
  return (
    ...
    >
      <div className="flex justify-between">
        <h3 className="font-bold" style={{ color: '#1D3557' }}>{task.title}</h3>
        {isAdmin && (
          <div className="space-x-2">
            <button className="btn btn-xs" style={{ backgroundColor: '#EFB917', color: '#122235', border: 'none' }} onClick={() => onEdit(task)} type="button">Edit</button>
            <button className="btn btn-xs btn-error" onClick={() => onDelete(task.taskId)} type="button">Delete</button>
          </div>
        )}
      </div>
      <p className="text-sm" style={{ color: '#122235' }}>{task.description}</p>
      <p className="text-xs mt-2" style={{ color: '#122235' }}>
        Project ID: <strong>{task.projectID}</strong> | Priority: <strong>{PriorityLevel[task.priority]}</strong> | Due: {task.dueDate ? new Date(task.dueDate).toLocaleString() : "N/A"}
      </p>
      <div className="flex items-center gap-3 mt-2 text-[#122235]">
        {task.imageUrl && (
          <PaperClipIcon className="h-4 w-4" title="Attachment" />
        )}
      ...
      {task.imageUrl && <img src={task.imageUrl} alt="attachment" className="mt-2 max-h-24 rounded" />}
    </div>
  );
};
...
const KanbanBoard: React.FC<KanbanBoardProps> = ({ isAdmin }) => {
    ...
    <div className="min-h-screen w-full pt-10">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 px-6 py-4 shadow-sm rounded-b-lg">
        ...
        <div className="flex gap-2 items-center">
          <select className="select select-bordered" value={selectedProjectId ?? "all"} onChange={handleFilterChange}>
        ...
        <div className="flex flex-col lg:flex-row gap-4 px-6 py-4">
          {columns.map((col) => (
            <DroppableColumn key={col.status} status={col.status}>
              <h2 className="text-xl font-semibold mb-4" style={{ color: '#2E6BFC' }}>{col.title}</h2>
