  ...
  return (
    <div className="h-screen overflow-y-auto flex justify-center items-start p-4 pt-20">
      ...
      <div className="w-full bg-[#FFFFFF] rounded-3xl shadow-xl border border-gray-200 p-8">
        ...
        <div className="mb-6">
          <h2 className="text-lg font-semibold text-blue-900 font-[Manrope] mb-2">📝 Description</h2>
        ...
        <div className="flex flex-wrap md:flex-nowrap items-end gap-6 mb-10 text-sm">
          <div className="w-32">
            ...
            <span className={`inline-block px-3 py-1 rounded-full font-medium text-xs ${priorityColor[task.priority]}`}>
        ...
        <div className="grid md:grid-cols-2 gap-10">
          <div>
            <h2 className="text-lg font-semibold text-blue-900 font-[Manrope] mb-2">💬 Comments</h2>
            {commentsLoading ? (
              ...
              <>
                {comments.length === 0 ? (
                  ...
                  <div className="space-y-4 mb-4">
                    {comments.map((c) => (
                      <div key={c.commentID} className="bg-gray-50 p-3 rounded-xl border border-gray-200">
                        {editId === c.commentID ? (
                          <>
                            <textarea
                              value={editContent}
                          ...
                          <>
                            ...
                            <div className="text-xs text-gray-400 mt-1 flex justify-between">
                              <span>
                                — {c.createdByUserName || 'Anonymous'} at {new Date(c.createdAt).toLocaleString()}
                              </span>
                              {c.createdByUserId === userId && (
                                <div className="space-x-2">
                                  ...
                                  <button onClick={() => handleDelete(c.commentID)} className="text-red-600 hover:underline text-xs">
                                    Delete
                                  </button>
                                </div>
                              )}
          ...
          <div>
            <h2 className="text-lg font-semibold text-blue-900 font-[Manrope] mb-4">🗂️ Subtasks</h2>
            {subtasks.length === 0 ? (
              <p className="text-sm italic text-gray-500">No subtasks for this task.</p>
            ) : (
              <ul className="space-y-4">
                {subtasks.map((subtask) => (
                  <li key={subtask.subTaskID} className="bg-gray-50 p-4 rounded-xl border border-gray-200">
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        <span className="font-semibold block">{subtask.title || subtask.name}</span>
                        <span className={`text-xs inline-block mt-1 px-2 py-1 rounded-full ${subtask.status === 100 ? 'bg-green-200 text-green-800' : 'bg-gray-200'}`}>
                          {subtask.status === 100 ? 'Complete' : 'Incomplete'}
                        </span>
                      </div>
                    </div>
                    <div className="text-sm text-gray-700 space-y-1 mb-2">
                      {subtask.description && <div><strong>Description:</strong> {subtask.description}</div>}
                      {subtask.dueDate && <div><strong>Due:</strong> {new Date(subtask.dueDate).toLocaleDateString()}</div>}
                      {subtask.priority && <div><strong>Priority:</strong> {subtask.priority}</div>}
                      {subtask.users?.length > 0 && (
                        <div><strong>Assigned:</strong> {subtask.users.map((u: any) => u.userName).join(', ')}</div>
                      )}
                    </div>
                          ...
                          return files.map((file: any, idx: number) => (
                            <div key={idx} className="relative group">
                              {/\.(jpg|jpeg|png|gif|bmp|webp)$/i.test(file.fileName) ? (
                                <a href={file.filePath} target="_blank" rel="noopener noreferrer" className="block">
                                  <img src={file.filePath} alt={file.fileName} className="w-24 h-24 object-cover rounded border mb-1" />
                                  <div className="text-xs text-gray-700 max-w-[96px] truncate text-center">
                                    {file.fileName}
                                  </div>
                                </a>
                              ) : (
                                <a href={file.filePath} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center">
                                  <FaFileAlt className="w-12 h-12 text-gray-400 mb-1" />
